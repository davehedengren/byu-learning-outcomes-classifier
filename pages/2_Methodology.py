...
*   **HTML Fetching:** Selenium was used by `course_scraper.py` to navigate to each course URL and download the raw HTML content of the page. Pages were saved locally (`course_pages/`) for parsing.
*   **Data Parsing:** BeautifulSoup was employed within `course_scraper.py` to parse the saved HTML files.
...
*   **Storage:** Extracted data for each learning outcome was stored by `course_scraper.py` as a row in a CSV file (`learning_outcomes_raw.csv`).
...
st.header("2. Data Cleaning (`clean_csv.py`)")
st.markdown("""
...
*   **Output:** Produced a cleaned CSV file (`data/learning_outcomes_cleaned.csv` or similar) ready for classification.
""")

st.header("3. Learning Outcome Classification (`classify_outcomes_batch.py`)")
st.markdown("""
...
*   **Setup:** Configured secure access to the OpenAI API via a `.env` file.
*   **Input:** The classification script (`classify_outcomes_batch.py`) reads the *cleaned* CSV data (e.g., `data/learning_outcomes_cleaned.csv`).
...
*   **Output Storage:**
    ...
    *   Saved the results incrementally to the final classified CSV file (e.g., `data/classified_learning_outcomes_cleaned.csv`).
""")

st.header("4. Dashboard Visualization (`dashboard.py`)")
st.markdown("""
Finally, this Streamlit dashboard (`dashboard.py`) was created to load the classified data (e.g., `data/classified_learning_outcomes_cleaned.csv`) and provide interactive visualizations and exploration capabilities, including filtering by college/department and viewing aim distributions.
""") 